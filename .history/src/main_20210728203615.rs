...
fn main() {
    ...
    let mut a = Trian {x:[20, 20], y:[40, 10]};
    let mut b = Trian {x: [40, 10], y: [60, 40]};
    let mut c = Trian {x: [60, 40], y: [20, 20]};

    win.line(a.x[0], a.x[1], a.y[0], a.y[1], WHITE);
    win.line(b.x[0], b.x[1], b.y[0], b.y[1], WHITE);
    win.line(c.x[0], c.x[1], c.y[0], c.y[1], WHITE);

    if a.y > b.y {swap(&mut a.y, &mut b.y);}
    if a.y > c.y {swap(&mut a.y, &mut c.y);}
    if b.y > c.y {swap(&mut b.y, &mut c.y);}
