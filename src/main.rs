...
fn main() {
    ...
    for facet in &face.facets {
        ...
        for i in 0..3 {
            ...
            let one = ((v0.x + 1.) * WIDTH as f32 /2.) as i32;
            let two = ((v0.y + 1.) * HEIGHT as f32 /2.) as i32;
...
fn triangle(mut a:  Vector2D, mut b: Vector2D, mut c: Vector2D,  win: &mut window::Screen, color: [u8; 3]) {
    ...
    for i in 0 .. total_height as i32 {
        ...
        // win.put_pixel(a_vec.x as i32, a.y as i32 + i, color);
        // win.put_pixel(b_vec.x as i32, a.y as i32 + i, color);
